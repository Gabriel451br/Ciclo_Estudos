/* =============================================
   CICLO DE ESTUDOS — app.js
   Timer corrigido com timestamps reais,
   bip sonoro via Web Audio API,
...
const CIRC   = 2 * Math.PI * 68; // raio=68
...
const DEFAULT_STATE = {
  ...
  cycleOrder:  null,   // gerado após load
  sessions:    [],
  simsGeral:   [],
  simsMateria: [],
  metas:       {},
...
// ─── POMODORO (estado em runtime, não persistido) ───
let pomoState = {
  phase:      'focus',  // 'focus' | 'short' | 'long'
  running:    false,
  startedAt:  null,     // Date.now() quando iniciou/retomou
  remaining:  0,        // segundos restantes ao iniciar/retomar
  total:      0,        // duração total da fase
  cycle:      0,        // contagem de focos concluídos (para dot)
  subject:    '',
  rafId:      null,
  lastTick:   null,
};
...
// ─── PERSISTÊNCIA ────────────────────────────
const STORAGE_KEY = 'cicloEstudos_v2';

function saveState() {
  try {
    // Salva cicleOrder junto com subjects
    const toSave = {
      subjects:    S.subjects,
...
function loadState() {
  try {
    ...
    // Mescla campos salvos preservando defaults
    S.subjects    = saved.subjects    || S.subjects;
    S.cycleOrder  = saved.cycleOrder  || null;
    ...
    // Reseta contador de hoje se for outro dia
...
function clearAllData() {
  ...
  S.cycleOrder = S.subjects.map((_,i) => i);
...
function getPrio(s) {
  ...
  return (s.pct / 100 * 0.6 + (1 - avg / 100) * 0.4) * (s.freq === 2 ? 1.2 : 1);
}

function totalPct() {
  return +S.subjects.reduce((a,b) => a + Number(b.pct), 0).toFixed(2);
}
...
function buildSelects() {
  ...
  const today = todayStr();
  if (!document.getElementById('sg-date').value)
...
function showTab(tab, btn) {
  ...
  if (tab === 'sims')   { renderSgChart(); renderSimMateriaChart(); renderMetaChart(); renderMetaProgress(); renderSmList(); renderSgList(); }
...
function updateMetrics() {
  ...
  const metaSec  = S.cfg.hpd * 3600;
  const pct      = Math.min(100, Math.round(todaySec / metaSec * 100));
...
function renderCharts() {
  ...
  charts.pie = new Chart(document.getElementById('pieChart'), {
    type: 'pie',
    data: {
      labels: pd.map(s => s.name),
      datasets: [{ data: pd.map(s => s.pct), backgroundColor: PIE_C.slice(0, pd.length), borderWidth: 2, borderColor: '#1a2234' }]
    },
    options: {
  ...
  charts.bar = new Chart(document.getElementById('barChart'), {
    type: 'bar',
    data: {
      labels: S.subjects.map(s => s.name.split(' ')[0]),
      datasets: [{ label:'h/sem', data: S.subjects.map(s => getRecHrs(s)), backgroundColor:'#4a8af455', borderColor:'#4a8af4', borderWidth:1 }]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend:{display:false} },
      scales: {
        x: { ticks:{ color:'#8a97b0', font:{size:10}, autoSkip:false, maxRotation:45 }, grid:{color:'rgba(255,255,255,.04)'} },
...
function addSimGeral() {
  ...
  const score = parseFloat(document.getElementById('sg-score').value);
  ...
  S.simsGeral.push({ date, score, obs });
  document.getElementById('sg-score').value = '';
...
function renderSgList() {
  ...
  el.innerHTML = [...S.simsGeral].reverse().map(s => {
    ...
    return `<div class="srow">
      <span style="color:var(--text)">Simulado geral${s.obs ? ' — ' + s.obs : ''}</span>
      <div style="display:flex;align-items:center;gap:7px">
        <span style="font-size:12px;color:var(--text2)">${d.getDate()}/${d.getMonth()+1}/${d.getFullYear()}</span>
        <span class="sp ${cls}">${s.score}%</span>
      </div>
    </div>`;
  }).join('');
}
...
function renderSgChart() {
  ...
  charts.sg = new Chart(document.getElementById('sgChart'), {
    type: 'line',
    data: {
      labels: sorted.map(s => { const d = new Date(s.date + 'T12:00:00'); return `${d.getDate()}/${d.getMonth()+1}`; }),
    ...
    options: {
      ...
      plugins:{ legend:{display:false} },
      scales:{
        y:{ min:0, max:100, ticks:{color:'#8a97b0'}, grid:{color:'rgba(255,255,255,.06)'}, title:{display:true,text:'%',color:'#8a97b0'} },
        x:{ ticks:{color:'#8a97b0'}, grid:{color:'rgba(255,255,255,.04)'} }
      }
    }
  });
}
...
function addSimMateria() {
  const sub   = document.getElementById('sm-sub').value;
  const score = parseFloat(document.getElementById('sm-score').value);
  const meta  = parseFloat(document.getElementById('sm-meta').value);
  ...
  document.getElementById('sm-score').value = '';
  document.getElementById('sm-meta').value  = '';
  renderSmList(); renderSimMateriaChart(); renderMetaProgress(); renderMetaChart();
  renderPrioList(); renderCycle(); updateMetrics(); saveState();
}
...
function renderSmList() {
  ...
  el.innerHTML = [...S.simsMateria].reverse().map(s => {
    ...
    return `<div class="srow">
      ...
      <div style="display:flex;align-items:center;gap:7px">
        <span style="font-size:12px;color:var(--text2)">${dt.getDate().toString().padStart(2,'0')}/${(dt.getMonth()+1).toString().padStart(2,'0')}</span>
        <span class="sp ${cls}">${s.score}%</span>
      </div>
    </div>`;
  }).join('');
...
function renderSimMateriaChart() {
  const data = S.subjects.map(s => {
    const sims = S.simsMateria.filter(x => x.sub === s.name);
    return { name: s.name.split(' ')[0], avg: sims.length ? Math.round(sims.reduce((a,b) => a+b.score,0)/sims.length) : null };
  }).filter(d => d.avg !== null);
  // (gráfico de barra de matéria foi integrado ao metaChart)
}
...
function renderMetaChart() {
  ...
  const avgs  = subjs.map(s => {
    const sims = S.simsMateria.filter(x => x.sub === s.name);
    return sims.length ? Math.round(sims.reduce((a,b) => a+b.score,0)/sims.length) : null;
  });
  charts.meta = new Chart(document.getElementById('metaChart'), {
    type: 'bar',
    data: {
      labels: subjs.map(s => s.name.split(' ')[0]),
      datasets: [
    ...
    options: {
      ...
      scales:{
        y:{ min:0, max:100, ticks:{color:'#8a97b0'}, grid:{color:'rgba(255,255,255,.06)'}, title:{display:true,text:'%',color:'#8a97b0'} },
        x:{ ticks:{color:'#8a97b0', font:{size:11}}, grid:{display:false} }
      }
    }
  });
}
...
function renderCycle() {
  const validOrder = (S.cycleOrder || []).filter(i => i < S.subjects.length);
  const ordered    = validOrder.map(i => S.subjects[i]);
  let cycle = [];
  ordered.forEach(s => { cycle.push({...s,rep:1}); if (s.freq===2) cycle.push({...s,rep:2}); });
  ...
  cycle.forEach((s, i) => {
    ...
    div.innerHTML = `
      ...
      <div style="flex:1;min-width:0">
        <div class="ci-name">${s.name}${s.freq===2?` <span style="font-size:11px;opacity:.7">(${s.rep}ª)</span>`:''}</div>
        <div class="ci-meta">${s.area} · ${getRecHrs(s)}h/sem${avg!==null?' · '+avg+'%':''}</div>
      </div>
      <span class="badge ${s.pct>=15?'bh':s.pct>=7?'bm':'bl'}">${s.pct>0?s.pct+'%':'Red.'}</span>`;
    ...
    div.addEventListener('drop', e => {
      ...
      const arr = cycle.map(c => S.subjects.findIndex(x => x.name === c.name));
      arr.splice(to, 0, arr.splice(from, 1)[0]);
      const seen = new Set(), newOrder = [];
      arr.forEach(idx => { if (!seen.has(idx)) { seen.add(idx); newOrder.push(idx); } });
      S.cycleOrder = newOrder;
...
function renderRevAlerts() {
  ...
  const items = Object.entries(last).map(([sub, date]) => {
    ...
    let level = null, msg = '';
    if      (days >= S.cfg.r3) { level='r3'; msg=`Revisão profunda recomendada (${days}d sem estudar)`; }
  ...
  if (!items.length) {
    el.innerHTML = '<div class="rev-alert rev-ok"><i class="ti ti-check"></i> Todas as matérias em dia! Continue assim.</div>';
    return;
  }

  const icons = { r1:'ti-clock', r2:'ti-bell', r3:'ti-alert-triangle' };
  el.innerHTML = items.map(x =>
    `<div class="rev-alert rev-${x.level}">
      <i class="ti ${icons[x.level]}"></i>
      <div><strong>${x.sub}</strong> — ${x.msg}</div>
    </div>`
  ).join('');
}
...
function renderSubjTable() {
  ...
  const el = document.getElementById('subj-table');
  el.innerHTML = '';
  S.subjects.forEach((s, i) => {
    const recH = (() => {
      const total = S.cfg.hpd * S.cfg.dpw;
      const sum   = S.subjects.reduce((a,b) => a + getPrio(b), 0);
      return sum ? Math.max(0.5, +((getPrio(s) / sum) * total).toFixed(1)) : 0;
    })();
    ...
    row.innerHTML = `
      ...
      <button class="btn btn-r btn-sm" onclick="removeSubject(${i})" aria-label="Remover ${s.name}">
...
function addSubject() {
  ...
  if (!name || isNaN(pct) || pct < 0) { alert('Preencha nome e porcentagem da matéria.'); return; }
  S.subjects.push({ name, pct, freq, area, customHrs: null });
  S.cycleOrder.push(S.subjects.length - 1);
  document.getElementById('ns-name').value = '';
  document.getElementById('ns-pct').value  = '';
  document.getElementById('ns-area').value = '';
  syncAll();
}
...
function removeSubject(i) {
  ...
  S.cycleOrder = S.subjects.map((_,j) => j);
  syncAll();
}

function syncAll() {
  if (!S.cycleOrder || S.cycleOrder.some(i => i >= S.subjects.length))
    S.cycleOrder = S.subjects.map((_,i) => i);
...
function renderSLog() {
  ...
  const today = new Date().toDateString();
  const todaySessions = S.sessions.filter(s => new Date(s.date).toDateString() === today);

  if (!todaySessions.length) {
    el.innerHTML = '<p class="empty-msg">Nenhuma sessão hoje. Complete um pomodoro!</p>';
    return;
  }
  el.innerHTML = [...todaySessions].reverse().map(s => {
    const min = Math.floor(s.seconds / 60);
    const dt  = new Date(s.date);
    const tag = s.phase === 'focus'
      ? '<span class="badge bh" style="font-size:10px">foco</span>'
      : '<span class="badge bl" style="font-size:10px">pausa</span>';
    return `<div class="srow">
      <span style="color:var(--text)">${s.subject} ${tag}</span>
      <span style="color:var(--text2);font-size:12px;font-family:var(--font-mono)">${min}min · ${dt.getHours().toString().padStart(2,'0')}:${dt.getMinutes().toString().padStart(2,'0')}</span>
    </div>`;
  }).join('');
}
...
function playBeep(type = 'focus') {
  try {
    ...
    const sequences = {
      focus: [  // 3 bips descendentes — fim do foco
        { freq:880, start:0,    dur:.12 },
        { freq:660, start:.16,  dur:.12 },
        { freq:440, start:.32,  dur:.25 },
      ],
      break: [  // 2 bips ascendentes — início de pausa
        { freq:440, start:0,    dur:.1 },
        { freq:660, start:.14,  dur:.2 },
      ]
    };
    const seq = sequences[type] || sequences.focus;
  ...
  } catch(e) { /* navegador sem suporte a AudioContext */ }
}

// ─── POMODORO — CORE ─────────────────────────
// Usa timestamps reais (Date.now) para não travar ao trocar de aba
// requestAnimationFrame garante atualização visual suave
...
function updatePomoDisplay(remaining) {
  ...
  const total = pomoState.total || 1;
  ring.style.strokeDashoffset = (CIRC * (sec / total)).toFixed(2);
}

function setPhaseDisplay() {
  const ph = document.getElementById('pomo-ph');
  const map = {
    focus: { label:'Foco 🎯',         color:'var(--accent)' },
    short: { label:'Pausa curta ☕',  color:'#639922'       },
    long:  { label:'Pausa longa 🌿',  color:'#d4941c'       }
  };
...
function updateDots() {
  for (let i = 0; i < 4; i++)
    document.getElementById('d' + i).classList.toggle('done', i < (pomoState.cycle % 4));
}
...
function resetPomo() {
  ...
  pomoState.total     = getFSec();
  pomoState.remaining = getFSec();
  pomoState.startedAt = null;
  setPhaseDisplay();
  updatePomoDisplay(pomoState.remaining);
...
function startPomo() {
  ...
  pomoState.subject   = sub;
  pomoState.running   = true;
  pomoState.startedAt = Date.now();
  // Se estava pausado, remaining já tem o valor correto
  if (!pomoState.startedAt || pomoState.remaining <= 0)
...
function phaseEnd() {
  ...
  const subject = pomoState.subject;
  const phase   = pomoState.phase;
  const seconds = pomoState.total;

  // Registra sessão
  S.sessions.push({ subject, seconds, date: Date.now(), phase });
  ...
  if (phase === 'focus') {
    ...
    updateDots();
    document.getElementById('pomo-cnt').textContent =
      `${S.pomoToday} pomodoro${S.pomoToday !== 1 ? 's' : ''} completo${S.pomoToday !== 1 ? 's' : ''} hoje`;

    const isLong = pomoState.cycle % 4 === 0;
    pomoState.phase     = isLong ? 'long' : 'short';
    pomoState.total     = isLong ? getLSec() : getSSec();
    pomoState.remaining = pomoState.total;
    document.getElementById('pomo-st').textContent = isLong
      ? '🌿 Pausa longa merecida! Descanse bem.'
      : '☕ Pomodoro concluído! Descanse um pouco.';
  } else {
    playBeep('break');
    pomoState.phase     = 'focus';
    pomoState.total     = getFSec();
...
function init() {
  ...
  // Garante cycleOrder
  if (!S.cycleOrder || !S.cycleOrder.length)
    S.cycleOrder = S.subjects.map((_,i) => i);
  ...
  // Renderiza charts com delay mínimo para garantir dimensões do canvas
